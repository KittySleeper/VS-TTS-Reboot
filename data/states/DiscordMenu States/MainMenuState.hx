...
function create() {
    ...
    var textServer = new FlxText(0,0, 0, "", 16);
    ...
    var borderSizing = {x: 10, y: 25};
    var colorWhyFlixel = 0xFF111214;

    var tabBG = new FlxSprite().makeSolid(textServer.fieldWidth + borderSizing.x, textServer.height + borderSizing.y, colorWhyFlixel);
    tabBG.shader = new CustomShader("bevelShader");
    tabBG.shader.corner_scale = 0.4;
    tabBG.onDraw = function(sprite:FlxSprite) {
        if (textServer.fieldWidth > 195) textServer.fieldWidth = 195;
        if (sprite.width != Std.int(textServer.fieldWidth + borderSizing.x)) {
            sprite.makeSolid(textServer.fieldWidth + borderSizing.x, textServer.height + borderSizing.y, colorWhyFlixel);
        }
...
function serverTabUI() {
    ...
    for (idx in 0...serverData.length) {
        ...
        icon.updateHitbox();
        icon.shader = new CustomShader("bevelShader");
        icon.shader.corner_scale = 1;
...
function userTabUI() {
    ...
    userIcon.shader = new CustomShader("bevelShader");
    userIcon.shader.corner_scale = 1;
...
function update(elapsed:Float) {
    ...
    for (idx in 0...serverIconGroup.members.length) {
        ...
        var cornerValue = (FlxG.mouse.overlaps(icon)) ? 0.5 : 1;
        icon.shader.corner_scale = FlxMath.lerp(icon.shader.corner_scale, cornerValue, elapsed*15);
